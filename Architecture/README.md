  ...
  Каждая команда - это класс с методом `execute`, который получает на вход текущий `Context` и возвращает результат выполнения: результирующую строку и код возврата.
    ...
    - grep – ищет заданный образец во входных данных и выводит строки, в которых он содержится <br>
      *Описание:*<br> Проходимся по каждой строке из входных данных (прочитали из файлов или получили выходные данные от другой команды через `context`) и ищем данный нам образец с помощью библиотеки `re`.<br>
      ...
      Разбор опций проводится с помощью модуля `argparse` – модуль простой в использовании и имеет нужную функциональность - это были главные критерии при отборе. Также у нас был опыт работы с данным модулем. Помимо этого рассматривались еще две библиотеки: `Docopt` и `Python Fire`.  
      ...
      `Python Fire` генерирует CLI интерфейс из любого pyhton-объекта. Крутая библиотека, но нужно много времени, чтобы с ней разобраться. К тому же создавать класс с методами занимает намного больше строчек кода. <br>
